...
bool readFileToMatrix(const std::string &filePath, Matrix &mat)
{
    ...
    if(!is.is_open())
    {
//        std::cerr << "first fail" << std::endl;
        return false;
    }
//    std::cout << "getters test(rows) :" << mat.getRows() << std::endl;
//    std::cout << "getters test(cols) :" << mat.getCols() << std::endl;
//    std::cout << "getters test(rows * cols ) :" << mat.getRows() * mat.getCols() << std::endl;



    long int matByteSize = (long int) mat.getCols() * mat.getRows() * sizeof(float);

//    std::cout << "size matByte is: " << matByteSize << std::endl;
//    std::cout << "size of float in bytes: " << sizeof(float) * 8 << std::endl;
//    std::cout << "avis calculation: " << 128 * 784 * 32 << std::endl;
